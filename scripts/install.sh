...
VERSION="${PLANE_CLI_VERSION:-latest}"
...
curl -fsSL "$asset_url" -o "$TMP_DIR/plane-cli.tar.gz"
tar -xzf "$TMP_DIR/plane-cli.tar.gz" -C "$TMP_DIR"

BIN_DIR="${BIN_DIR:-$HOME/.local/bin}"
mkdir -p "$BIN_DIR"
install -m 755 "$TMP_DIR/plane" "$BIN_DIR/plane"

echo "installed: $BIN_DIR/plane"
